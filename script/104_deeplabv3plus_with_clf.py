...
#             param[1].requires_grad = False
    ...
    train_params = [{'params': net.get_1x_lr_params(), 'lr': LearningRate},#/10
                    {'params': net.get_10x_lr_params(), 'lr': LearningRate}]#*10
...
MODEL = 'resnet' #'xception'#'resnet' #backbone for deeplab
...
checkpoint_path = '../checkpoint/deeplabv3plus_%s_%dx%d_v13_seed%s'%(MODEL, IMG_SIZE[0], IMG_SIZE[1], SEED)
LOG_PATH = '../logging/deeplabv3plus_%s_%dx%d_v13_seed%s.log'%(MODEL, IMG_SIZE[0], IMG_SIZE[1], SEED)
#torch.cuda.manual_seed_all(SEED)
